...
<%
	...
	if login_u_type=3 then
		...
		if rs.bof and rs.eof then
		   ...
		   if rs("St_agree") = false then 
		     response.Write("<script>alert('您注册后还没有得到批准，批准通过后再行登录！请返回……');history.back()</script>")
		     response.end
		   else 
	...
	'if login_user_no=user_no and login_user_pass=user_pass then
	   ...
	   if u_type=1 or u_type=4 then
	   	     response.redirect "../admin/bnuzitc.asp"
		end if

		if u_type=2 then
	   		response.redirect "../teacher/bnuzitc.asp"
		end if
		if u_type=3 then
	   		response.redirect "../student/bnuzitc.asp"
		end if
