...
<br />
  ...
  <%
		...
		sql = "select * from select_true selt,select_cursor sel,teacher_inf tea,student_info stu where selt.Tea_id=tea.Tea_id and  selt.St_number=stu.St_number and selt.sel_flag=false and selt.sel_id=sel.sel_id "&wh&" order by sel.Sel_id asc"
		'response.Write(sql)
  ...
  <tr bgcolor="#FFFFFF" onmouseover="this.bgColor='f1f1f1'" onmouseout="this.bgColor='FFFFFF'"  style='cursor:hand'  title="<%=rsobj("Sel_con")%>">
    ...
    <td><a href="task_add.asp?Result=Modify&show=yes&ID=<%=rsobj("sel.Sel_id")%>&txtpage=<%=page%>"
